...
export default class GlobalAccess {
  models = ['owner', 'animal']; // * instead of an array will allow access to all models
  
  // Custom logic here
  access(request) {
    const { url } = request; // destructure url from express request object

    // Returning false flat out denies access
    if (url.endsWith('/owner/angela')) return false;
    ...
    if (url.endsWith('/owner')) return record => record.id !== 'angela';
    if (url.endsWith('/animal')) return record => record.owner !== 'angela';
