// test/helpers/pg-test-helper.ts
import pg from 'pg';
 ...
 */
export function setupPgTests(hooks: any, { tables = [] as string[] } = {}) {
  ...
  hooks.before(async function () {
    ...
    try {
      const client = new (pg as any).Client(TEST_PG_CONFIG);
    ...
    // Create pool
    pool = new (pg as any).Pool(TEST_PG_CONFIG);
