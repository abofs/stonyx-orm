// test/helpers/mysql-test-helper.ts
 ...
 */
export function setupMysqlTests(hooks: any, { tables = [] as string[] } = {}) {
  ...
  hooks.before(async function () {
    ...
    try {
      const conn = await (mysql as any).createConnection(TEST_MYSQL_CONFIG);
      await conn.end();
    } catch {
    ...
    // Create pool
    pool = (mysql as any).createPool(TEST_MYSQL_CONFIG);

    // Reset MysqlDB singleton
    (MysqlDB as any).instance = null;
